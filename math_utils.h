...
static void mat4_perspective(float* m, float fov, float aspect, float n, float f) {
    float S = 1.0f / tanf(fov * 0.5f); memset(m, 0, 64);
    m[0]=S/aspect; m[5]=S; m[10]=(f+n)/(n-f); m[11]=-1.0f; m[14]=(2.0f*f*n)/(n-f);
}
static void mat4_rotate_y(float* m, float a) { mat4_identity(m); m[0]=cosf(a); m[2]=-sinf(a); m[8]=sinf(a); m[10]=cosf(a); }
static void mat4_rotate_x(float* m, float a) { mat4_identity(m); m[5]=cosf(a); m[6]=sinf(a); m[9]=-sinf(a); m[10]=cosf(a); }
...
static void mat4_lookat(float* m, const float* pos, float pitch, float yaw) {
    ...
    mat4_rotate_y(rotY, yaw); mat4_rotate_x(rotX, pitch);
    mat4_mul(tmp, rotY, trans); mat4_mul(m, rotX, tmp);
}
#endif
