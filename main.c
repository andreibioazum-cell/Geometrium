...
static void engine_handle_cmd(struct android_app* app, int32_t cmd) {
    ...
    if (cmd == APP_CMD_INIT_WINDOW) {
        ...
        const char* fS = 
            ...
            "  vec4 tc;"
            "  int t = int(vType + 0.5);"
            "  if(t == 3){ tc = texture2D(texLeaves, vUV); if(tc.a < 0.45) discard; }" // Листья
            "  else if(t == 2){ if(vNorm.y > 0.5 || vNorm.y < -0.5) tc = texture2D(texTreeTop, vUV); else tc = texture2D(texTreeSide, vUV); }" // Дерево
            "  else { if(vNorm.y > 0.5) tc = texture2D(texTop, vUV); else if(vNorm.y < -0.5) tc = texture2D(texDown, vUV); else tc = texture2D(texSide, vUV); }" // Трава
            "  float light = clamp(dot(vNorm, normalize(vec3(0.4, 1.0, 0.3))) * 0.6 + 0.5, 0.4, 1.0);"
        ...
        game_seed = (unsigned int)time(NULL); eng->worldSeed = (int)game_seed;
    }
}
...
void android_main(struct android_app* state) {
    ...
    eng->app = state; eng->movePointerId = -1; eng->lookPointerId = -1;
