...
struct engine {
    ...
    float camPos[3], camRot[2]; // x,y,z и pitch,yaw
    float touchX, touchY;
    bool touching;
};
...
static int32_t engine_handle_input(struct android_app* app, AInputEvent* event) {
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if (x < eng->width / 2) { // Левая сторона - ходьба
            eng->camPos[0] += sinf(eng->camRot[1]) * 0.1f;
            eng->camPos[2] -= cosf(eng->camRot[1]) * 0.1f;
        } else { // Правая сторона - обзор
...
// ... (инициализация шейдеров такая же, как была) ...

void android_main(struct android_app* state) {
    struct engine eng = {0};
    eng.camPos[1] = 5.0f; eng.camPos[2] = 5.0f; // Начальная позиция
    generate_world();
    
    state->userData = &eng;
    state->onAppCmd = engine_handle_cmd;
    state->onInputEvent = engine_handle_input; // ПОДКЛЮЧАЕМ ВВОД
    eng.app = state;

    while (1) {
        int id, ev; struct android_poll_source* s;
        while ((id = ALooper_pollOnce(eng.touching ? 0 : -1, NULL, &ev, (void**)&s)) >= 0) {
