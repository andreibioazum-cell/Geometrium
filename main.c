...
static void engine_handle_cmd(struct android_app* app, int32_t cmd) {
    ...
    if (cmd == APP_CMD_INIT_WINDOW) {
        ...
        const char* vS = "attribute vec3 pos; attribute vec2 uv; attribute vec3 norm; varying vec2 vUV; varying vec3 vNorm; uniform mat4 m; void main(){ gl_Position=m*vec4(pos,1.0); vUV=uv; vNorm=norm; }";
        const char* fS = "precision mediump float; varying vec2 vUV; varying vec3 vNorm; uniform sampler2D texTop, texSide, texDown; void main(){ vec4 tc; if(vNorm.y > 0.5) tc = texture2D(texTop, vUV); else if(vNorm.y < -0.5) tc = texture2D(texDown, vUV); else tc = texture2D(texSide, vUV); float light = clamp(dot(vNorm, normalize(vec3(0.4, 1.0, 0.3))) * 0.6 + 0.5, 0.3, 1.0); gl_FragColor = vec4(tc.rgb * light, 1.0); }";
        ...
        glBindAttribLocation(eng->program, 0, "pos"); glBindAttribLocation(eng->program, 1, "uv"); glBindAttribLocation(eng->program, 2, "norm");
...
void android_main(struct android_app* state) {
    ...
    while (1) {
        ...
        while (ALooper_pollOnce(0, NULL, &ev, (void**)&s) >= 0) {
            ...
            if (state->destroyRequested) { free(eng); return; }
        }
