...
static void init_inv_shader(struct engine* eng) {
    const char* vS =
        ...
        "  float iso_x = (aPos.x - aPos.z) * 0.7071;"
        "  float iso_y = (aPos.x + aPos.z) * 0.4082 + aPos.y * 0.8165;"
        "  gl_Position = vec4(iso_x * scale / aspect + offset.x,"
        "                     iso_y * scale + offset.y, 0.0, 1.0);"
        "  vUV = aUV;"
        "  vFace = aFace;"
        "}";
    ...
    const char* fS =
        ...
        "  if(face == 0) col = texture2D(texTop, vUV);"
        "  else col = texture2D(texSide, vUV);"
        /* Грани разной яркости */
        "  float light = 1.0;"
        "  if(face == 1) light = 0.7;"
        "  if(face == 2) light = 0.85;"
